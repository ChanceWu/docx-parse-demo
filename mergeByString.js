...
async function mergeDocx() {
    ...
    const data1Xml = fs.readFileSync(dPath1, 'utf-8');
    const data1Body = data1Xml.match(/<w:body>(\.*)<\/w:body>/);
    console.log(data1Body);
    return;
    baseXml.replace(/(<\/w:body><\/w:document>)/, data1Body+'$1');
    console.log(baseXml);
    var xmlTest = builder.buildObject({...baseXml});
